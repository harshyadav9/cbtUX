...
import Subheader from "./header/subHeader.js";
...
const useStyles = (theme) => ({
  ...
  legendContainer: {
    ...
    marginLeft: 19,
    boxShadow:
      "0px 4px 13px -1px rgb(0 0 0 / 20%), 0px 1px 10px 0px rgb(0 0 0 / 14%), 0px 1px 20px 0px rgb(0 0 0 / 19%)",
  },
  legends: {
    alignItems: "center",
    display: "flex",
...
function Test(props) {
  ...
  useEffect(() => {
    ...
    if (res.data.unansweredFlagsCount.length > 0) {
      localStorage.setItem(
        "unansweredArr",
        JSON.stringify(res.data.unansweredFlagsCount)
      );
    } else {
    ...
    if (res.data.answeredFlagsCount.length > 0) {
      localStorage.setItem(
        "questionNo",
        JSON.stringify(res.data.answeredFlagsCount)
      );
    ...
    if (
      answeredFlagsCount.length > 0 &&
      reviewAnsweredFlagsCount.length > 0 &&
      reviewUnansweredFlagsCount.length > 0
    ) {
    ...
    dispatch({
      type: "ADDINFO",
      hours: res.data.durationHr,
      minutes: res.data.durationMin,
      photo: res.data.photo,
      name: res.data.instituteName,
  ...
  const loadButtonStateInIntermittent = (questions) => {
    ...
    for (let i = 0; i < questions.questionList.length; i++) {
      ...
      buttonObj[i + 1] = {};
      if (
        questions.reviewAnsweredFlagsCount.indexOf(
          questions.questionList[i]["questionNo"]
        ) > -1
      ) {
        defaultCaseFlag = false;
        buttonObj[i + 1]["answer"] =
          questions.questionList[i]["selectedAnswer"];
      ...
      if (
        questions.reviewUnansweredFlagsCount.indexOf(
          questions.questionList[i]["questionNo"]
        ) > -1
      ) {
        defaultCaseFlag = false;
      ...
      if (
        questions.answeredFlagsCount.indexOf(
          questions.questionList[i]["questionNo"]
        ) > -1
      ) {
        defaultCaseFlag = false;
      ...
      if (
        questions.unansweredFlagsCount.indexOf(
          questions.questionList[i]["questionNo"]
        ) > -1
      ) {
        defaultCaseFlag = false;
    ...
    for (let i = 0; i < questions.length; i++) {
      if (questions[i]["selectedAnswer"]) {
        ...
        if (questions[i]["multiple"] === "N") {
          stateObj[questions[i]["questionNo"]]["key"] =
            questions[i]["selectedAnswer"];
          stateObj[questions[i]["questionNo"]]["value"] =
            questions[i]["selectedAnswer"];
        } else {
          ...
          for (let j = 0; j < checkBoxAnswersArr.length; j++) {
            myMap.set(
              `${checkBoxAnswersArr[j]}-${questions[i]["questionNo"]}`,
              true
            );
          }
  ...
  const reviewAndNext = () => {
    ...
    if (
      ...
      state.questions[activeQues] === undefined ||
      state.questions[activeQues]["key"] === ""
    ) {
      ...
      if (
        ...
        if (reviewQues === null) {
          ...
          setOptionSelected({
            questionNo: activeQues,
            option: state.questions[activeQues]["key"],
          });
          setquestionNo({ id: activeQues, type: "reviewA" });
          localStorage.setItem("reviewQuestionNo", JSON.stringify(obj));
        }
