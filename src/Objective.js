...
function RadioButtonsGroup(props) {
    ...
    useEffect(() => {
        ...
        if ((objective &&  objective.questionNo === (activeStep + 1)) && state.questions[activeStep + 1] !== undefined && !state.reset) {
            ...
            if (typeof (state.questions[activeStep + 1].value) === "string") {
                ...
                if (state.questions[activeStep + 1].value !== "") {
                    setRadio(prevState => ({
                        ...prevState,
                        radio: `${state.questions[activeStep + 1].value}-${objective.questionNo}` 
                    }));
                } else {
                    setRadio(prevState => ({
                        ...prevState,
                ...
                // if (state.idToSave.indexOf(activeStep + 1) > -1) {
                    ...
                    // console.log("newMap",newMap)
                        ...
                        for (let i = 0; i < 4; i++) {
                            let obj = {};
                            arr.set(`${state.questions[activeStep + 1].value}-${objective.questionNo}`, false);
                        }
                        setRadio(prevState => ({
                            ...prevState,
                            checkbox: arr
        ...
        if (state.reset) {
            ...
            if (objective  && objective.questionNo === (activeStep + 1)) {
                ...
                if (objective.multiple === 'N') {
                    ...
                    state.questions[objective.questionNo] = { ...state.questions[objective.questionNo], key: "", value: "" };
                    ...
                    state.questions[objective.questionNo] = { ...state.questions[objective.questionNo], key: "", value: newMap };
    ...
    const handleChange1 = (eventval, option, objective) => {
        ...
        console.log("radioval" , radioval);
        let obj = { ...state.questions, ...{ [objective.questionNo]: { key: option, value: option  } } };
        console.log("obj",obj);
    ...
    const handleChange = (isChecked, name, objective) => {
        ...
        let obj = { ...questions, ...{ [objective.questionNo]: { ...prevValueObj , key: checkBoxValues , value: arr} } };
        dispatch({ type: 'ADD_QUESTION_VALUES', questions: obj, reset: false });
        ...
        <div className={classes.root}>
            <div>
                ...
                <FormControl component="fieldset" className={classes.formControl}>
                    <FormLabel component="legend"><span className={classes.quesNo}>{objective  && objective.questionNo})</span><span className={classes.quesText}>{objective && objective.question}</span></FormLabel>
                    ...
                    >
                        <FormControlLabel value={`${objective.option1.split('-')[0]}-${objective.questionNo}`} onChange={(e) => handleChange1(e.target.value, objective.option1.split('-')[0], objective)} control={<Radio />} label={objective.option1} />
                        <FormControlLabel value={`${objective.option2.split('-')[0]}-${objective.questionNo}`} onChange={(e) => handleChange1(e.target.value, objective.option2.split('-')[0], objective)} control={<Radio />} label={objective.option2} />
                        {objective.option3 === undefined ? null
                            :
                            <FormControlLabel value={`${objective.option3.split('-')[0]}-${objective.questionNo}`} onChange={(e) => handleChange1(e.target.value, objective.option3.split('-')[0], objective)} control={<Radio />} label={objective.option3} />}
                        {objective.option4 === undefined ? null :
                            <FormControlLabel value={`${objective.option4.split('-')[0]}-${objective.questionNo}`} onChange={(e) => handleChange1(e.target.value, objective.option4.split('-')[0], objective)} control={<Radio />} label={objective.option4}
                            />}
                    </RadioGroup>) : (
                            <FormGroup>
                              {(objective && objective.option1 &&  <FormControlLabel
                                    control={<Checkbox checked={!!radioval.checkbox.get(`${objective.option1.split('-')[0]}-${objective.questionNo}`)} onChange={(e) => handleChange(e.target.checked, objective.option1.split('-')[0], objective)} name={objective && objective.option1} />}
                                    label={objective && objective.option1}
                                />)}
                              {( objective && objective.option2 && <FormControlLabel
                                    control={<Checkbox checked={!!radioval.checkbox.get(`${objective.option2.split('-')[0]}-${objective.questionNo}`)} onChange={(e) => handleChange(e.target.checked, objective.option2.split('-')[0], objective)} name={objective && objective.option2} />}
                                    label={objective && objective.option2}
                                />)}
                               {( objective && objective.option3 && <FormControlLabel
                                    control={<Checkbox checked={!!radioval.checkbox.get(`${objective.option3.split('-')[0]}-${objective.questionNo}`)} onChange={(e) => handleChange(e.target.checked, objective.option3.split('-')[0], objective)} name={objective && objective.option3} />}
                                    label={objective && objective.option3}
                                />)}
                               {(  objective && objective.option4 && <FormControlLabel
                                    control={<Checkbox checked={!!radioval.checkbox.get(`${objective.option4.split('-')[0]}-${objective.questionNo}`)} onChange={(e) => handleChange(e.target.checked, objective.option4.split('-')[0], objective)} name={objective && objective.option4} />}
